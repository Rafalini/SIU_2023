...
from src.turtlesim.turtlesim_env_base import TurtlesimEnvBase
from src.turtlesim.dqn_single import DqnSingle
...
class DqnMulti(DqnSingle):
    ...
    def train_main(self, save_model=True, save_state=True):
        ...
        last_states = {tname: agent.map for tname, agent in
                       self.env.agents.items()}  # poprzednie stytuacje początkowo takie same
        ...
        while episode < self.EPISODES_MAX:  # ucz w epizodach treningowych
            ...
            for tname in to_restart:  # odczytanie sytuacji
                ...
                if (episode + 1) % self.SAVE_MODEL_EVERY == 0 and save_model:
                    ...
                    self.model.save(f"models/model-Multi-E{episode}-{current_timestamp_ms}.tf", save_format="tf")  # zapisz model w formacie h5
            ...
            for tname, (new_state, reward, done) in self.env.agents.items():  # obsługa po kroku dla każdego agenta
